...
typedef struct VideoState {
    ...
    enum ShowMode {
        SHOW_MODE_NONE = -1, SHOW_MODE_VIDEO = 0, SHOW_MODE_NB
    } show_mode;
...
static int configure_video_filters(AVFilterGraph *graph, VideoState *is, AVFrame *frame)
{
...
#define INSERT_FILT(name, arg) do {                                          \
    ...
    if ((ret = configure_filtergraph(graph, filt_src, last_filter)) < 0)
        goto fail;
...
static int video_thread(void *arg)
{
    ...
    for (;;) {
        ...
        if (   last_w != frame->width
            ...
            if ((ret = configure_video_filters(graph, is, frame)) < 0) {
                SDL_Event event;
...
//      of the seek_pos/seek_rel variables
        ...
        if (is->audioq.size + is->videoq.size > MAX_QUEUE_SIZE ||
            (stream_has_enough_packets(is->audio_st, is->audio_stream, &is->audioq) && stream_has_enough_packets(is->video_st, is->video_stream, &is->videoq))
        ) {
            /* wait 10 ms */
